...
class ChessTrainer:
    def __init__(self, network: ValueNetwork, device: torch.device = None,
                 ...
                 output_dir: str = './data/models/self_play'):
        """
        Args:
...
def full_training_pipeline(output_dir: str = './data/models/self_play'):
    """
    Full pipeline: Self-play → Training
    
    Args:
        output_dir: Directory để lưu model (mặc định: ./data/models/self_play)
    """
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Lưu training data
    os.makedirs('./data/datasets/self_play', exist_ok=True)
    train_data_path = './data/datasets/self_play/training_data.npz'
    ...
    # Trainer
    trainer = ChessTrainer(network, device=device, learning_rate=0.001, output_dir=output_dir)
