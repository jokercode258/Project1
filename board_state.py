"""
BƯỚC 1: BIỂU DIỄN TRẠNG THÁI BÀN CỜ (STATE REPRESENTATION)
12 planes, mỗi plane 8x8 cho 12 loại quân cờ
"""
...
class BoardState:
    ...
    def board_to_tensor(board: chess.Board) -> np.ndarray:
        ...
        for square in chess.SQUARES:
            ...
            # Tính hàng và cột
            row = 7 - (square // 8)  # Từ trên xuống dưới
            col = square % 8           # Trái sang phải
    ...
    def tensor_to_board(tensor: np.ndarray) -> chess.Board:
        ...
        Args:
            tensor: numpy array shape (12, 8, 8)
            
        Returns:
            chess.Board object
        """
        board = chess.Board(fen='8/8/8/8/8/8/8/8 w KQkq - 0 1')  # Board rỗng
        board.clear()
    ...
    def board_to_fen(tensor: np.ndarray, is_white_turn: bool) -> str:
        """
        Chuyển tensor → FEN string (để debug)
        """
